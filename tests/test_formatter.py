...
def _base(stage, **kw):
    return OrchestrationResult(
        stage=stage, question="q", model_name="fake-model",
        total_time_ms=12.3, **kw,
    )
...
def test_completed():
    ex = ExecutionResult(
        ...
        columns=("a",), rows=((1,),), rows_returned=1,
        truncated=False, execution_time_ms=2.0,
    )
...
def test_completed_truncated_mentions_limit():
    ex = ExecutionResult(
        status=ExecutionStatus.SUCCESS, executed_sql="SELECT 1",
        columns=("a",), rows=tuple((i,) for i in range(5)),
        rows_returned=5, truncated=True, execution_time_ms=1.0,
    )
...
def test_no_query():
    r = format_response(_base(PipelineStage.NO_QUERY_PROPOSED,
                              model_message="I can't answer that."))
...
def test_validation_rejected():
    ...
    r = format_response(_base(PipelineStage.VALIDATION_REJECTED,
                              candidate_sql="SELECT * FROM X", validation=vr))
    assert not r.success
...
def test_execution_failed():
    ex = ExecutionResult(
        ...
        execution_time_ms=1.0, error_message="Query execution failed: no column bad",
    )
    vr = ValidationResult.approve("SELECT bad FROM Fact_Orders LIMIT 100")
    r = format_response(_base(PipelineStage.EXECUTION_FAILED,
                              candidate_sql="SELECT bad FROM Fact_Orders",
                              validation=vr, execution=ex,
...
def test_api_error_does_not_leak_internals():
    r = format_response(_base(PipelineStage.API_ERROR,
                              error_message="401 Unauthorized: bad key sk-secret"))
    assert not r.success
...
def test_purity_same_input_same_output():
    ex = ExecutionResult(status=ExecutionStatus.SUCCESS, executed_sql="SELECT 1",
                         columns=("a",), rows=((1,),), rows_returned=1,
...
def test_to_dict_is_json_safe():
    ...
    import json
    ex = ExecutionResult(status=ExecutionStatus.SUCCESS, executed_sql="SELECT 1",
                         columns=("rev",), rows=((Decimal("250.00"),),),
                         rows_returned=1, truncated=False, execution_time_ms=1.0)
