...
def schema(tmp_path: Path):
    ...
    con.execute(
        "CREATE TABLE Fact_Orders (order_key INTEGER, customer_key INTEGER, date_key INTEGER, net_revenue DECIMAL(12,2))"
    )
    con.close()
